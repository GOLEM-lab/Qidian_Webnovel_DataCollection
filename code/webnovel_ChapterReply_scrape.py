...
def get_ChapterReplies(bookId,chapterId,reviewId,lastReviewId,pageIndex):
    '''all parameters are strings. pageIndex start from 1 and lastreviewId start from 0.'''
    url = "https://www.webnovel.com/go/pcm/chapterReview/getReviewReplys"

    querystring = {"_csrfToken":"4dbce8fb-ea3d-49d8-936a-b82dac7812cf","bookId":bookId,"chapterId":chapterId,"reviewId":reviewId,"lastReviewId":lastReviewId,"pageIndex":pageIndex,"_":"1722951500218"}
    ...
    json_ChapterReplies = json_response['data']['chapterReplyItems']
...
if __name__ == '__main__':
    ...
    for chapterId in tqdm(chapterIds[7:]):
        dfReviews = pd.read_csv('data/webnovelReviews_Chapter_ByChapter/' + bookId + '/' + chapterId + '.csv',dtype={'reviewId':'string'})
        dfReviewsWithReplies = dfReviews[dfReviews['replyAmount']!=0]
        reviewIds = dfReviewsWithReplies['reviewId'].values.tolist()
        print(len(reviewIds))
        ...
        for reviewId in reviewIds: 
            ...
            while isLast =='0':
                time.sleep(1)
                dfTemp,isLast,lastReviewId = get_ChapterReplies(bookId,chapterId,reviewId,lastReviewId,pageIndex)
                dfChapter = pd.concat([dfChapter,dfTemp],ignore_index=True)
                pageIndex_int +=1
                pageIndex = str(pageIndex_int)
                lastReviewId = str(lastReviewId)
        dfChapter.to_csv('data/webnovelReplies_Chapter_ByChapter/' + bookId + '/' + chapterId + '.csv',index=False)